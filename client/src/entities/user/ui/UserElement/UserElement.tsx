...
import cls from './UserElement.module.scss'

interface UserElementProps {
  user: IUser
}

export const UserElement: FC<UserElementProps> = ({ user }) => {
  console.log(user.id)

  return (
    <>
      <div className={cls.user}>
        <div className={cls.id}>1 {user.id}</div>
        <div className={cls.username}>{user.username}</div>
        <div className={cls.orderQty}>3</div>
